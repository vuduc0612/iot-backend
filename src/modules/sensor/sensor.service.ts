...
export class SensorService {
  ...
  async saveSensorData(message: any): Promise<Sensor> {
    ...
    // Kiểm tra nếu null thì gán giá trị = 0
    const temperature = Math.round(parsedMessage.temperature || 0);
    const humidity = Math.round(parsedMessage.humidity || 0);
    const light = Math.round(parsedMessage.light || 0);
